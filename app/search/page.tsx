...
export default function SearchMembersPage() {
    ...
    const [membersSearchResults, setMembersSearchResults] = useState<Record<string, any>[]>([]);
    ...
    useEffect(() => {
        const setup = async () => {
            ...
            if (roles.includes("admin") || roles.includes("registrar") || roles.includes("treasurer")) {
        ...
        <div>
            ...
            <div className="flex flex-col items-center space-x-10 w-full justify-center">
                <div className="flex items-center w-3/5 p-4">
                    <label htmlFor="search" className="text-sm font-semibold mr-4">Search Members</label>
                    <Input id="search" type="text" placeholder="Search member by any metric" className="flex-grow mr-4" value={searchTerm} 
                    ...
                    <Button id = "search-button" className="bg-blue-600 hover:bg-blue-700 text-white" onClick={() => handleGoogleSearch(searchTerm)} >Search</Button>
                </div>
                
                <div className="mt-6 flex justify-center space-x-4">
                        <Button className="bg-blue-600 hover:bg-blue-700 text-white"
                        onClick={() => handleShowDetails(selectedMemberId, entries, router)}>
                            Show Details
                        </Button>
                        {
                        selectedView?.upsert_function &&
                            <Button variant="outline" className="border-blue-300 bg-blue-300 text-white hover:bg-blue-500 hover:text-white mb-8"
                                onClick={() => handleEditDetails(selectedMemberId, entries, selectedView, router)}>
                                Edit Details
                            </Button>
                        }
                    </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                {
                                selectedView && Object.entries(selectedView.schema.columns).map(([name, _]) => (
                                    <TableHead className="text-lg font-bold text-black" key={name}>{name}</TableHead>
                                ))
                                }
                            </TableRow>
                        </TableHeader>
