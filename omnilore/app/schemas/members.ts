...
export const memberViews: View[] = [
  ...
  },
  {
    ...
    upsert_function: async (value: any): Promise<any> => {
      const { data, error } = await supabase.from("members").upsert(value);
      if (error) throw error;
      return data;
    },
...
export async function createMembershipReportsQueryFunction(
  fromDate: Date,
  toDate: Date
): Promise<any> {
  ...
  const { data: orders, error: ordersError } = await supabase
    .from("orders")
    .select(
      "user_names, user_emails, date, skus, user_amounts, fee, payment_platform, sqsp_transaction_id, sqsp_order_id, external_transaction_id, created_at, updated_at, amount"
    )
