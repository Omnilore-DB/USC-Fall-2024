  ...
  return (
    <div style={{ marginTop: '40px' }}>
      ...
      <table style={tableStyle}>
        ...
        <tbody>
          {['grossReceipts', 'fees', 'netReceipts', 'percentage'].map((field, idx) => (
            <tr key={idx}>
              <td style={rowHeaderStyle}>
                {field === 'grossReceipts' ? 'Gross Receipts'
                 : field === 'fees' ? 'Fees'
                 : field === 'netReceipts' ? 'Net Receipts'
                 : 'Percentage'}
              </td>
              {data.map((row, i) => (
                <td style={tdStyle} key={i}>
                  {field === 'percentage'
                    ? row.percentage
                    : formatCurrency(row[field as keyof StripeData] as number)}
                </td>
              ))}
              <td style={tdStyle}>
                {field === 'percentage' ? '--' : formatCurrency(
                  field === 'grossReceipts' ? totals.gross
                  : field === 'fees' ? totals.fees
                  : totals.net
                )}
              </td>
            </tr>
