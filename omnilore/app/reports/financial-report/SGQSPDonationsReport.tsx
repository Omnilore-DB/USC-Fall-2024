  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      const combinedData = membersToTransactions.map((mtt) => {
        ...
        const transaction = transactions.find((t) => t.sqsp_transaction_id === mtt.transaction_id);

        // Split street_address into address1 and address2 (address2 for Apt/unit)
        const [address1, ...address2Parts] = (member?.street_address || '').split(',');
        ...
        if (aptMatch) {
          ...
          return {
            ...
            year_month: transaction ? formatDateToYearMonth(transaction.created_at) : '', // Ensure Year-Month is derived from created_at
  ...
  return (
    <div style={mainContainerStyle}>
      ...
      <div style={inputContainerStyle}>
        ...
        <button onClick={handleFetch} style={generateButtonStyle}>Generate Report</button>
        <button onClick={handleExport} style={exportButtonStyle}>Export to Excel</button>
      </div>
...
const inputContainerStyle = { display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' };
const generateButtonStyle = { backgroundColor: 'green', color: 'white', padding: '5px 10px' };
const exportButtonStyle = { backgroundColor: 'royalblue', color: 'white', padding: '5px 10px' };
const tableContainerStyle = { marginTop: '20px' };
const tableStyle = { width: '100%', borderCollapse: 'collapse', border: '1px solid #ddd' };
const thStyle = { border: '1px solid #ddd', padding: '10px', backgroundColor: '#f2f2f2', fontWeight: 'bold', textAlign: 'center' };
const tdStyle = { border: '1px solid #ddd', padding: '10px', textAlign: 'center' };
