...
const ProductBreakdown = () => {
  ...
  const processProductBreakdown = (data: any[]) => {
    ...
    data.forEach(tx => {
      ...
      if (!grouped[product][platform]) {
        grouped[product][platform] = Array(12).fill(null).map(() => ({ gross: 0, fee: 0 }));
      }
    ...
    Object.keys(grouped).forEach(product => {
      ...
      platforms.forEach(platform => {
        const monthly = grouped[product][platform] || Array(12).fill({ gross: 0, fee: 0 });
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold mb-4">Product Breakdown - 2024</h1>
            ...
            <thead>
              <tr>
                {tableData[0]?.map((header, idx) => (
                  <th key={idx} className="border px-4 py-2 font-semibold text-left">{header}</th>
                ))}
              </tr>
            </thead>
            <tbody>
