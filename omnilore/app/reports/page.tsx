...
function groupDataByCriteria(data: Record<string, any>[], criteria: string): DashboardItem[] {
    ...
    const dashboardData: DashboardItem[] = Object.entries(groupCounts).map(([group, count]) => ({
        group: group,
        count: count,
...
export default function MemberSearchComponent() {
    ...
    const exportMembershipReportToExcel = async () => {
        ...
        const skuDescriptions: { [key: string]: string } = products.reduce((acc, product) => {
            acc[product.sku] = product.description;
            return acc;
        }, {} as { [key: string]: string });

        let ordersSheetData: any[] = [
            ['user_names', 'user_emails', 'date', 'SKU_description', 'skus', 'user_amounts', 'fee', 'sqsp_transaction_id', 'sqsp_order_id', 'payment_platform', 'external_transaction_id', ]
        ];

    
            ordersSheetData = ordersSheetData.concat(entries.map(entry => [
                entry.user_names,
                entry.user_emails,
                entry.date,
            ...
            const dashboardSheetData = [
                ...
                ...dashboardData.map(({ group, count, percentage, totalAmount, sku_description }) => [
                    sku_description,
                    group,
                    count,
                    percentage,
    ...
    const exportForumReportToExcel = async () => {
        ...
        const selectedEntry = entries.find(entry => entry.pid === selectedMemberId);
        if (!selectedEntry) {
        ...
        try {
            ...
            const { data: ordersData, error } = await supabase
                .from("orders")
                .select("created_at, updated_at, sqsp_transaction_id, sqsp_order_id, user_emails, amount, date, skus, payment_platform, fee, external_transaction_id, user_names, user_amounts")
                .contains('skus', [sku])
            ...
            // Prepare orders sheet data with split rows
            const columnHeaders = [
                'user_names', 'user_emails', 'date', 'skus', 'user_amounts', 'fee', 'sqsp_transaction_id', 'sqsp_order_id', 'payment_platform', 'external_transaction_id'
            ];
            let ordersSheetData: any[] = [columnHeaders];

            
            let ordersDashboardData: Record<string, { count: number, totalAmount: number }> = {};
    
            const filteredOrdersData = filter_splitOrdersBySKU(ordersData, [sku]);

            ordersSheetData = ordersSheetData.concat(filteredOrdersData.map(order => [
                order.user_names,
                order.user_emails,
                order.date,
                order.skus,
            ...
            const total = Object.values(ordersDashboardData).reduce((sum, { count }) => sum + count, 0);

            const criteria = selectedReport?.name === "Membership" ? "skus" : "payment_platform";
            ...
            <main className="flex flex-row">
                ...
                <div className="flex flex-col items-center space-x-10 w-full justify-center">
                        ...
                        <div className="mt-4 flex flex-col space-y-4">
                            ...
                            <div className="flex space-x-4">
                                <div className="relative">
                                    <label>From:</label>
                                    <div className="flex items-center border border-gray-300 rounded-md p-2" onClick={() => document.getElementById('fromDatePicker')?.focus()}>
                                        <DatePicker
                                            selected={fromDate}
                                ...
                                <div className="relative">
                                    <label>To:</label>
                                    <div className="flex items-center border border-gray-300 rounded-md p-2" onClick={() => document.getElementById('toDatePicker')?.focus()}>
                                        <DatePicker
                                            selected={toDate}
                    ...
                    <div className="mt-6 flex justify-center space-x-4">
                        ...
                        <Button
                            onClick={selectedReport?.name === "Membership" ? exportMembershipReportToExcel: exportForumReportToExcel}
                            disabled={
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        {
                                            selectedReport && Object.entries(selectedReport.schema.columns).map(([name, _]) => (
                                                ...
                                                {selectedReport?.name !== "Membership" && (
                                                    <TableCell>
                                                        <Checkbox
                                                            ...
                                                            disabled={selectedMemberId !== "" && selectedMemberId !== entry.pid}
                                                            className={selectedMemberId !== "" && selectedMemberId !== entry.pid ? "bg-gray-600" : ""}
                                                        />
                                                    </TableCell>
                                                )}
                                                {
